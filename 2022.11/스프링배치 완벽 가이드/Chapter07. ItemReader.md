...
## 에러 처리
시작, 처리 중 또는 결과를 기록할 때와 같이 문제가 발생하는 시점에 따라 나눌 수 있음  
그 중 처리 중 발생하는 에러를 다루는 방법에 대해 살펴보고자 함

### 레코드 건너뛰기
- 어떤 예외를 건너뛰게 할 지, 몇 번까지 건너뛰게 할 지 정해야 함
  - skip, skipLimit을 StepBuilderFactory에서 지정 가능
- SkipPolicy라는 인터페이스의 shouldSkip 메서드 구현

### 잘못된 레코드 로그 남기기
- ItemListener를 사용하여 잘못된 레코드 기록
  - beforeRead, afterRead, onReadError 메서드로 구성

### 입력이 없을 때의 처리
- 별다른 설정이 없을 경우에는 입력 파일에서 null이 반환 되더라도 스텝이 완료되었다고 처리됨
  - 입력이 없을 때 이를 알아챌 수 있어야 하는 상황이라면 별도의 설정이 필요
- StepListener 사용
  - afterStep 메서드에서 입력이 없는 경우 COMPLETED 이외의 상태로 끝나게 설정하여 추후 잡을 재실행 가능케 함
  - 스텝을 실패로 처리하는 것 외에도 이메일을 보내는 것 같은 다른 처리도 가능